...
public class CustomerLoginSuccessHadler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		// 성공시 LoginServlet에서 authorities에 저장된 값을 확인한 후에
		// 권한에 해당하는 페이지 이동
		// authentication.getAuithorities()함수가
		// LoginService에서 authorities에 저장된 값을 확인할 수 있음
		System.out.println("성공시 : " + authentication.getAuthorities());
		//아래코드를 실행하는 이유는 authoritie.getAuthority()문자열이 아니므로 문자열로 변환
		List<String> roleNames=new ArrayList<String>();
		authentication.getAuthorities().forEach(authoritie-> {
			roleNames.add(authoritie.getAuthority()); // role갑만 가져옴
		});
